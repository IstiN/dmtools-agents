# Teammate (`story_description.json`)

Writes a full story description from the ticket context and linked questions; output goes to the Description field by default or to Confluence when contentOutput is configured.

## Attributes

| Attribute | Value |
|---|---|
| ContextId | `—` |
| outputType | `none` |
| skipAIProcessing | `true` |

## Prompt snapshot

Full prompt / instruction set: [`agents/snapshots/story_description.md`](agents/snapshots/story_description.md)

## Actions


### preCliJSAction: `preCliContentOutputSetup.js`

_Pre-CLI Content Output Setup — shared pre-action for content-generating_

- Source: `agents/js/preCliContentOutputSetup.js`
- No detected file I/O or side effects.


### postJSAction: `writeContentOutput.js`

_Write Content Output — unified post-action for content-generating agents_

- Source: `agents/js/writeContentOutput.js`
- Side effects:
  - post Jira comment
- Parameters (customParams):
  - `contentOutput`


## LLM step

- outputType: `none`
- Prompt snapshot: `agents/snapshots/story_description.md`

## Custom params

- `contentOutput`: `{"target":"jira_field","field":"Description","operationType":"replace"}`
- `contentOutput` _(used by JS action)_

---
_Generated by js/agentDocGenerator.js_